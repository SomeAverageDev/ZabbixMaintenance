...
# create
zabbix_sender -c /etc/zabbix/zabbix_agentd.conf -s "Zabbix server" -k ZabbixMaintenance -o "action=create;duration=10;comment=test maintenance A test-host1 with 1 tag with contains value;host=test-host1;tag=tag1:contains1:2"
sleep 3
zabbix_sender -c /etc/zabbix/zabbix_agentd.conf -s "Zabbix server" -k ZabbixMaintenance -o "action=create;duration=20;comment=test maintenance B test-host2 with 2 tags;host=test-host2;tag=CAP_CATEGORY:OS;tag=tag2:contains2:2"
sleep 3
...
sleep 20
zabbix_sender -c /etc/zabbix/zabbix_agentd.conf -s "Zabbix server" -k ZabbixMaintenance -o "action=create;comment=test maintenance F with no duration;host=test-host2"
sleep 20
zabbix_sender -c /etc/zabbix/zabbix_agentd.conf -s "Zabbix server" -k ZabbixMaintenance -o "action=create;host=test-host3"
sleep 20

#delete
zabbix_sender -c /etc/zabbix/zabbix_agentd.conf -s "Zabbix server" -k ZabbixMaintenance -o "action=delete;host=test-host1;tag=tag1:contains1:2"
sleep 3
